<style>
  @import '/src/assets/style/adminlayout.css';
</style>
<template>
  <div class="sidebar-container">

    <!-- Sidebar -->
    <!-- Sử dụng Vue.js binding (:class) để gán class "
    collapsed" một cách động dựa trên giá trị của isCollapsed. -->
    <!-- Binding trong Vue.js là cách chúng ta liên kết (ràng buộc)
     dữ liệu từ JavaScript (data) vào HTML một cách động. -->
    <div class="sidebar d-flex flex-column flex-shrink-0 p-4 bg-light" :class="{ 'collapsed': isCollapsed }">
      <div class="sidebar-header">
        <button @click="toggleSidebar" class="btn toggle-btn">
          <font-awesome-icon :icon="['fas', 'bars']" />
        </button>
        <a href="/dashboard" class="d-flex align-items-center mb-4 text-dark text-decoration-none">
          <svg v-if="!isCollapsed" class="bi me-3 w-280" viewBox="0 0 190 32" height="32"
      ...
      <ul class="nav nav-pills flex-column mb-auto">
        <li v-for="(item, index) in menuItems" :key="index">
          <router-link :to="resolvePath(item.path)" class="nav-link d-flex align-items-center"
            ...
            <font-awesome-icon :icon="item.icon" class="me-3 fs-5" />
            <span v-show="!isCollapsed" class="fs-5">{{ item.label }}</span>
          </router-link>
        </li>
      </ul>
      ...
      <div class="dropdown">
        <a href="#" class="d-flex align-items-center link-dark text-decoration-none dropdown-toggle" id="dropdownUser2"
          ...
          <strong class="fs-5" v-show="!isCollapsed">mdo</strong>
        </a>
